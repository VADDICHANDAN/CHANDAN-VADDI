...
-- Dump completed
-- note : the final answer for the queries is between -- **************   answer    -**************

-- 1.	Show the percentage of wins of each bidder in the order of highest to lowest percentage.
create view v4 as (select *,count(bidder_id) over(partition by bidder_id)
 total_bids from ipl_bidding_details );
create view v5 as (select *,count(bidder_id) over(partition by bidder_id) 
bids_won from ipl_bidding_details where bid_status like 'won');
select v4.bidder_id,total_bids,v5.bidder_id,bids_won,
ifnull(bids_won,0)/total_bids*100 win_percentage 
from v4 left join v5 on v4.bidder_id=v5.bidder_id 
group by v4.bidder_id order by win_percentage desc;

--  ************************* ***********************------
SELECT ibd.BIDDER_ID, COUNT(ibd.BID_STATUS) , NO_OF_BIDS,
(COUNT(ibd.BID_STATUS)/NO_OF_BIDS)*100 AS PERCENTAGE_WIN
FROM ipl_bidding_details ibd
INNER JOIN ipl_bidder_points ibp
ON ibd.BIDDER_ID = ibp.BIDDER_ID
AND ibd.BID_STATUS ='Won'
...
-- ---****************************************************
select m.STADIUM_ID,STADIUM_NAME,CITY,count(m.stadium_id) cnt from 
ipl_match_schedule m join ipl_stadium s on m.STADIUM_ID = s.STADIUM_ID
where status = 'completed' group by stadium_id order by cnt desc;
...
select t.TEAM_NAME, BID_TEAM,count(bidder_id) total_bids from ipl_bidding_details bd 
join ipl_team t on bd.BID_TEAM = t.TEAM_ID 
group by BID_TEAM order by total_bids desc;

-- *********************************************************
SELECT COUNT(BID_TEAM) total_bids ,BID_TEAM,IPL_TEAM.TEAM_NAME
...
-- *********************************************************************
with subquery as;
-- ( select *,substr(win_details,instr(win_details,'m')+2
-- instr(win_details,'won')-instr(win_details,'m')-3) as Winner 
-- from IPL_match)
-- select match_id,team_id,i2.team_name from subquery s join ipl_team i2 
-- on i2.remarks like s.winner;
with subquery as
( select *,substr(win_details,instr(win_details,'m')+2,instr(win_details,'won')-instr(win_details,'m')-3) as Winner from IPL_match)
select match_id,team_id,Winner from subquery s join ipl_team i2 on i2.remarks like s.winner;
-- ******************************************************************on i2.remarks like s.winner

-- 6.	Display total matches played, total matches won and total matches lost by the team along with its team name.
select * from ipl_team_standings;  order by TOURNMT_ID;
select * from ipl_team;
select its.tournmt_id,it.team_name, matches_played, matches_won, matches_lost
from ipl_team_standings as its join ipl_team as it
...
select TEAM_ID,ipl_team_players.player_id,player_name, PLAYER_ROLE from ipl_team_players join ipl_player on ipl_player.PLAYER_ID=ipl_team_players.player_id where player_role 
like '%Bowler%' and team_id =(select team_id from ipl_team where team_name like '%Mumbai Indians%');

-- or

-- ***************************************************
select TEAM_ID, PLAYER_ID, PLAYER_ROLE from ipl_team_players where player_role 
like '%Bowler%' and team_id =
(select team_id from ipl_team where team_name like '%Mum%');
-- *************************************************************
...
-- *********************************************
select tp.TEAM_ID,PLAYER_ID, PLAYER_ROLE,tp.REMARKS,TEAM_NAME,count(PLAYER_ROLE) cnt 
from ipl_team_players tp join ipl_team t on tp.TEAM_ID =  t.TEAM_ID
where player_role like '%ALL-Rounder%' 
group by tp.team_id having cnt >4 order by cnt desc ;
-- *****************************************************************
...
-- ***************************************************************
with subquery as (select * from ipl_bidding_details where SCHEDULE_ID in  
(select schedule_id from ipl_match_schedule where stadium_id =7 and 
match_id in(select MATCH_ID from ipl_match where WIN_DETAILS like '%CSK won%')))
select bid_status,year(bid_date) as year,sum(TOTAL_POINTS) total_bidders_points 
from subquery join ipl_bidder_points on ipl_bidder_points.BIDDER_ID=subquery.bidder_id 
group by subquery.bid_status;

-- or

with subquery as 
(select * from ipl_bidding_details where SCHEDULE_ID in  
(select schedule_id from ipl_match_schedule where stadium_id =7 and match_id in
(select MATCH_ID from ipl_match where WIN_DETAILS like '%CSK won%')))
select bid_status,year(bid_date) as year,sum(TOTAL_POINTS) total_bidders_points 
from subquery join ipl_bidder_points on ipl_bidder_points.BIDDER_ID=subquery.bidder_id group by subquery.bid_status;
...
-- **********************************************************************
with outersubquery as (with subquery as (select player_id,player_name, 
cast(substr(PERFORMANCE_DTLS,instr(PERFORMANCE_DTLS,'wkt')+4,2) as float) wickets
from ipl_player where PLAYER_ID in ( select PLAYER_ID from ipl_team_players 
where PLAYER_ROLE like '%All-Rounder%' or PLAYER_ROLE like '%bowler%') ),
player_details as (select ipl_team.team_id,team_name,player_role, player_id 
from ipl_team, ipl_team_players where ipl_team.team_id=ipl_team_players.team_id )
select  player_name,player_role,subquery.player_id,player_details.team_name,wickets,
dense_rank() over(order by wickets desc) as wicket_rankings
from subquery,player_details where subquery.player_id=player_details.player_id)
select * from outersubquery where wicket_rankings<6;
-- ****************************************************
...
-- ********************************************************************
with subquery as
(select TOURNMT_ID,TOURNMT_NAME,FROM_DATE,TO_DATE,datediff(TO_DATE,FROM_DATE) duration_in_days,  
dense_rank() over(order by datediff(TO_DATE,FROM_DATE)/24 desc) D_Rank,
dense_rank() over(order by datediff(TO_DATE,FROM_DATE)/24 ) A_Rank
from ipl_tournament )
select TOURNMT_ID,TOURNMT_NAME,concat(FROM_DATE,'   TO  ',TO_DATE) duration_column, duration_in_days,
case when D_rank=1 then 'MAX Duration'  when A_rank =1 then 'MIN Duration' end as Remarks from subquery 
where D_rank =1 or A_rank = 1;
-- ***************************************************************************
...
select  * from ipl_bidding_details where year(bid_date)=2017;
select * from ipl_bidder_details;
-- **************************************************************************************
select bidder_details.BIDDER_ID,BIDDER_NAME,
date_format(BID_DATE,'%Y') bid_yr,
date_format(BID_DATE,'%m') bid_m,TOTAL_POINTS
from ipl_bidder_details bidder_details 
join ipl_bidding_details bidding_details on 
bidder_details.bidder_id=bidding_details.BIDDER_ID
join ipl_bidder_points bidder_points on 
bidder_points.BIDDER_ID = bidding_details.BIDDER_ID 
where date_format(BID_DATE,'%Y') like '%2017%' 
group by bidder_id order by bid_m,TOTAL_POINTS desc ;
-- *************************************************************************************
select * from ipl_match;
-- 14.	Write a query for the above question using sub queries by having the same constraints as the above question.
 ...
 select  * from ipl_bidding_details;
 -- ******************************************
select ipl_bidder_details.BIDDER_ID,bidder_name,
(select TOTAL_POINTS from ipl_bidder_points
 where ipl_bidder_details.BIDDER_ID=ipl_bidder_points.BIDDER_ID) TOTAL_POINTS,
date_format(BID_DATE,'%Y') bid_yr,date_format(BID_DATE,'%m') bid_m 
from ipl_bidder_details,ipl_bidding_details 
where ipl_bidder_details.BIDDER_ID=ipl_bidding_details.BIDDER_ID 
and date_format(BID_DATE,'%Y') like '%2017%' group by bidder_id  ;
-- *****************************************************
...
-- Bidder Id, Ranks (optional), Total points, Highest_3_Bidders --> columns contains name of bidder, Lowest_3_Bidders  --> columns contains name of bidder;
 ...
 -- **********************************************************************************************
with subquery as
(select ipl_bidder_points.BIDDER_ID,TOTAL_POINTS ,BIDDER_NAME,ipl_match_schedule.TOURNMT_ID,
dense_rank()over(order by total_points desc) D_rank,
dense_rank()over(order by total_points ) A_rank
 from ipl_bidder_points join ipl_bidder_details 
 on   ipl_bidder_points.BIDDER_ID = ipl_bidder_details.BIDDER_ID
 join ipl_bidding_details on ipl_bidding_details.BIDDER_ID = ipl_bidder_details.BIDDER_ID 
join ipl_match_schedule on ipl_bidding_details.SCHEDULE_ID = ipl_match_schedule.SCHEDULE_ID
where ipl_match_schedule.TOURNMT_ID =2018)
select BIDDER_ID,TOTAL_POINTS ,BIDDER_NAME,TOURNMT_ID , 
case when D_rank < 4 then 'Highest 3 Bidders'
when A_rank < 4 then 'Lowest 3 Bidders' end as Remarks
from subquery where A_rank < 4 or D_rank < 4 ;
-- *********************************************************************************** 
...
-- Assume you are working in an Ed-tech company namely Great Learning where you will be inserting and modifying the details of
-- the students in the Student details table. Every time the students changed their details like mobile number, 
-- You need to update their details in the student details table.  Here is one thing you should ensure whenever 
-- the new students' details come , you should also store them in the Student backup table so that if you modify -
-- the details in the student details table, you will be having the old details safely.
...
(109,'Chandan','Chandan@gmail',9010);
delimiter $$
create trigger backup
after insert on Student_details for each row
begin
